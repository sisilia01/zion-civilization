...
CRISIS_UNEMPLOYMENT_TRIGGER = 80.0
...
def should_end_crisis(metrics: dict[str, Any]) -> bool:
    return (
        ...
        and metrics["unemployment_rate"] < CRISIS_END_UNEMPLOYMENT
    )
...
def _log_power_event(cur, event_type: str, desc: str, scores: dict, outcome: str):
    cur.execute(
        """
    ...
    log_event(cur, None, "power_struggle", desc, 0, priority="breaking")
...
def run_power_struggles(cur, scores: dict[str, float]) -> str | None:
    ...
    if scores["senate_power"] > scores["president_power"] and laws_blocked > 3:
        desc = f"⚖️ Senate blocks President {pname} — approval and sheriff budget cut!"
