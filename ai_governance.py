...
def get_constitutional_context() -> str:
    """Текущие действующие конституционные параметры — вставляется в system prompt каждой фракции."""
    from amendment_enforcer import get_param

    params = {
        "top_tax_rate": float(get_param("top_tax_rate", 0.35) or 0.35),
        "min_tax_rate": float(get_param("min_tax_rate", 0.05) or 0.05),
        "wealth_tax_rate": float(get_param("wealth_tax_rate", 0.0) or 0.0),
        "corporate_tax_rate": float(get_param("corporate_tax_rate", 0.10) or 0.10),
        "cooling_off_days": float(get_param("cooling_off_days", 0) or 0),
        "term_limit_days": float(get_param("term_limit_days", 30) or 30),
        "emergency_unemployment_exit_threshold": float(
            get_param("emergency_unemployment_exit_threshold", 60) or 60
        ),
        "basic_income": float(get_param("basic_income", 0) or 0),
    }

    return f"""CURRENT CONSTITUTIONAL PARAMETERS (legally binding, you MUST act consistently with these):
- Top tax rate: {params['top_tax_rate']*100:.0f}%
- Minimum tax rate: {params['min_tax_rate']*100:.0f}%
- Wealth tax rate: {params['wealth_tax_rate']*100:.0f}%
- Corporate tax rate: {params['corporate_tax_rate']*100:.0f}%
- Cooling-off period (between government roles): {params['cooling_off_days']:.0f} days
- Term limit: {params['term_limit_days']:.0f} days
- Emergency state exits when unemployment drops below: {params['emergency_unemployment_exit_threshold']:.0f}%
- Universal basic income: {params['basic_income']:.0f} ZION

Your decisions MUST be consistent with these CURRENT values —
do not assume outdated rates. If tax is already at maximum,
do not propose raising it further."""


def get_recent_amendments_context(cur) -> str:
    """Последние 5 принятых поправок — чтобы AI знал что изменилось недавно."""
    cur.execute(
        """
        SELECT title, description FROM amendments
    ...
    lines = [
        "RECENTLY ENACTED CONSTITUTIONAL AMENDMENTS (you must respect these):"
    ]
    for row in rows:
        title = row["title"] if isinstance(row, dict) else row[0]
        desc = (row["description"] if isinstance(row, dict) else row[1]) or ""
...
def build_system_prompt(
    ...
    try:
        ...
        try:
            recent_amendments = get_recent_amendments_context(cur)
        finally:
            cur.close()
            conn.close()
    except Exception:
    ...
    prompt_prefix = constitutional_context
...
You are President {p.get('name', 'vacant')} of ZION — {pol['name']}. Approval: {p.get('approval', 0):.0f}%.
...
You are Sheriff {s.get('name', 'vacant')} — a constitutional law enforcement officer (Article XVII).
CRITICAL ALERTS: {alerts_text}
...
You are the ZION Senate — Speaker leads the {pol['name']} caucus.
CRITICAL ALERTS: {alerts_text}
...
{base_memory}"""
    ...
    if faction == "zrs_chief":
        ...
        exit_threshold = float(get_param("emergency_unemployment_exit_threshold", 60) or 60)
        return f"""{prompt_prefix}

You are the ZRS Chief - the independent central bank of ZION.
...
Current state:
- Unemployment: {unemployment_rate:.1f}% (CRISIS if > {exit_threshold:.0f}%)
- Inflation: {inflation_rate:.1f}%
- ZRS Reserve: {zrs_reserve:,.0f} ZION
- Average corp treasury: {avg_corp_treasury:,.0f} ZION

If unemployment > {exit_threshold:.0f}%: give_money to corporations so they hire
...
You are ZION Criminal Underground (clan activity tracked under civil law).
CRITICAL ALERTS: {alerts_text}
...
You are ZION Corporate Alliance.
CRITICAL ALERTS: {alerts_text}
...
{base_memory}"""
    ...
    return (
        f"{prompt_prefix}\n\n"
        f"You are the AI controller for {faction}.\n\n{SUPREME_CONSTITUTIONAL_LAW}\n\n{base_memory}"
    )
...
"""
        ...
        for attempt in range(2):
            try:
                ...
                print(
                    f"AI decide error for {faction} (attempt {attempt + 1}): {e}",
                    flush=True,
                )
                if attempt == 0:
    ...
    try:
        ...
        if action == "give_money" and amount > 0:
                ...
                if spend > 0:
                    ...
                    if cur.rowcount > 0:
                        cur.execute(
                            ...
                            """,
                            (corp_spend, corp_spend),
                        )
                        if cur.rowcount:
                            approval_delta = 4
                            adjust_president_approval(cur, approval_delta)
                            result += f" | Corporate stimulus {corp_spend:.0f} ZION (no direct welfare)"
                        else:
                            result += " | SKIPPED: insufficient personal_fund"
                    else:
                        result += " | SKIPPED: no corporations eligible (all treasury >= 500)"
                else:
                    result += " | SKIPPED: insufficient personal_fund"
            else:
            ...
            if spend > 0:
                ...
                if cur.rowcount > 0:
                    ...
                    if cur.rowcount:
                        approval_delta = 4
                        adjust_president_approval(cur, approval_delta)
                        result += f" | Stimulus {corp_spend:.0f} ZION to struggling corps (fund debited)"
                    else:
                        result += " | SKIPPED: insufficient personal_fund"
                else:
                    result += " | SKIPPED: no corporations eligible (all treasury >= 500)"
            ...
            try:
                ...
                pres_row = cur.fetchone()
                proposed_by = pres_row[0] if pres_row else None
                aid = propose_amendment(title, desc, change_type, proposed_by=proposed_by)
                if aid:
                    cur.execute(
                        """
                        INSERT INTO events (event_type, description, created_at)
