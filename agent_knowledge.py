...
LOCK_FILE = "/tmp/agent_knowledge_running.lock"
...
def read_chunk_cycle(batch: int = 30) -> int:
    ...
    if os.path.exists(LOCK_FILE):
        print("[agent_knowledge] previous cycle still running, skipping")
        return 0
    try:
        with open(LOCK_FILE, "w") as f:
            f.write(str(os.getpid()))
        return _read_chunk_cycle_impl(batch)
    finally:
        if os.path.exists(LOCK_FILE):
            os.remove(LOCK_FILE)


def _read_chunk_cycle_impl(batch: int) -> int:
    """Each agent reads one unread chunk in core or specialty track."""
