...
import {
  ...
  dusdcToBaseUnits,
  estimateQuantityForBudget,
  extractPredictManagerId,
  formatDusdcBaseUnits,
  getDusdcCoins,
...
type CivMarketIconConfig = {
  Icon: LucideIcon;
  bg: string;
  fg: string;
};

const CIV_FALLBACK_ICONS: CivMarketIconConfig[] = [
  { Icon: Calendar, bg: "#1e3a5f", fg: "#93c5fd" },
  { Icon: User, bg: "#334155", fg: "#e2e8f0" },
  { Icon: Landmark, bg: "#1a365d", fg: "#90cdf4" },
  { Icon: Trophy, bg: "#3d2f1f", fg: "#fbbf24" },
];

function civMarketIconFallback(title: string): CivMarketIconConfig {
  let h = 0;
  for (let i = 0; i < title.length; i++) h = (h * 31 + title.charCodeAt(i)) | 0;
  return CIV_FALLBACK_ICONS[Math.abs(h) % CIV_FALLBACK_ICONS.length];
}

function getCivMarketIconConfig(title: string, category = ""): CivMarketIconConfig {
  const t = (title || "").toLowerCase();
  const c = (category || "").toLowerCase();

  if (t.includes("police") && (t.includes("arrest") || t.includes("gang leader"))) {
    return { Icon: ShieldCheck, bg: "#1e40af", fg: "#ffffff" };
  }
  if ((t.includes("gang") && t.includes("rob")) || (t.includes("rob") && t.includes("corporation"))) {
    return { Icon: ShieldAlert, bg: "#7f1d1d", fg: "#fecaca" };
  }
  if (t.includes("revolution meter") || (t.includes("revolution") && t.includes("hit"))) {
    return { Icon: Flame, bg: "#ea580c", fg: "#ffffff" };
  }
  if (t.includes("president") && (t.includes("survive") || t.includes("overthrown"))) {
    return { Icon: Crown, bg: "#b45309", fg: "#fef3c7" };
  }
  if (t.includes("sheriff") && t.includes("survive")) {
    return { Icon: Shield, bg: "#1e40af", fg: "#ffffff" };
  }
  if (t.includes("bankrupt") || (t.includes("corporation") && t.includes("bankrupt"))) {
    return { Icon: Building2, bg: "#dc2626", fg: "#ffffff" };
  }
  if (
    t.includes("clan war") ||
    t.includes("which clan will win") ||
    t.includes("dominate") ||
    (t.includes("clan") && t.includes("win"))
  ) {
    return { Icon: Swords, bg: "#c2410c", fg: "#ffffff" };
  }
  if (
    t.includes("agents die") ||
    (t.includes("agents") && t.includes("die")) ||
    /\b(death|die|dies|died|killed)\b/.test(t)
  ) {
    return { Icon: Skull, bg: "#374151", fg: "#e5e7eb" };
  }
  if (
    t.includes("economic crisis") ||
    t.includes("hyperinflation") ||
    t.includes("inflation")
  ) {
    return { Icon: TrendingDown, bg: "#dc2626", fg: "#ffffff" };
  }
  if (t.includes("z-perps") || (t.includes("profit") && t.includes("class"))) {
    return { Icon: TrendingUp, bg: "#16a34a", fg: "#ffffff" };
  }
  if ((t.includes("new agents") && t.includes("born")) || (t.includes("born") && t.includes("agents"))) {
    return { Icon: Baby, bg: "#0ea5e9", fg: "#ffffff" };
  }
  if (t.includes("lose") && t.includes("members")) {
    return { Icon: UserMinus, bg: "#6b7280", fg: "#ffffff" };
  }
  if (t.includes("rebellion") || t.includes("revolution")) {
    return { Icon: Flame, bg: "#ea580c", fg: "#ffffff" };
  }

  if (c === "clans") return { Icon: Swords, bg: "#c2410c", fg: "#ffffff" };
  if (c === "crime") {
    if (t.includes("police") || t.includes("arrest")) {
      return { Icon: ShieldCheck, bg: "#1e40af", fg: "#ffffff" };
    }
    return { Icon: ShieldAlert, bg: "#7f1d1d", fg: "#fecaca" };
  }
  if (c === "economy") {
    if (t.includes("bankrupt")) return { Icon: Building2, bg: "#dc2626", fg: "#ffffff" };
    return { Icon: TrendingDown, bg: "#dc2626", fg: "#ffffff" };
  }
  if (c === "trading") return { Icon: TrendingUp, bg: "#16a34a", fg: "#ffffff" };
  if (c === "demographics") {
    if (t.includes("born")) return { Icon: Baby, bg: "#0ea5e9", fg: "#ffffff" };
    if (t.includes("lose") || t.includes("members")) {
      return { Icon: UserMinus, bg: "#6b7280", fg: "#ffffff" };
    }
    return { Icon: Skull, bg: "#374151", fg: "#e5e7eb" };
  ...
  if (c === "politics") {
    if (t.includes("president")) return { Icon: Crown, bg: "#b45309", fg: "#fef3c7" };
    if (t.includes("sheriff")) return { Icon: Shield, bg: "#1e40af", fg: "#ffffff" };
    return { Icon: Flame, bg: "#ea580c", fg: "#ffffff" };
  }

  return civMarketIconFallback(title);
}

const CivMarketIcon = ({ title, category }: { title: string; category?: string }) => {
  const { Icon, bg, fg } = getCivMarketIconConfig(title, category || "");
  return (
    <div
      style={{
        width: 40,
...
function zionMarketRowToApiMarket(market: ZionMarketRow): ZionbetApiMarket {
  return {
    ...
    created_at: market.created_at ?? null,
    description: market.description || null,
    timeframe: zionbetTimeframeFromMarketDates(market.expires_at, market.created_at),
    image_url: null,
  };
}
...
function zionbetApiToMarket(m: ZionbetApiMarket): ZionBetMarket {
  ...
  const et = (m.event_type || m.id || "").toLowerCase();
  const isCgUpdown =
    Boolean(m.token) &&
    (et.startsWith("btc_") || et.startsWith("eth_") || et.startsWith("sui_"));
  return {
    id: m.id,
    question: m.question,
    ...
    ...(isCgUpdown ? { market_kind: "updown" as const } : {}),
  };
}
...
type ZionBetToastPayload =
  | string
  | { message: string; disclaimer?: string; type?: "success" | "warning" | "error"; silent?: boolean };
...
const ZIONBET_CURRENCY_MENU_BG = "#0c1a30";

function zionbetCurrencyDropdownPanel(extra?: CSSProperties): CSSProperties {
  return {
    background: ZIONBET_CURRENCY_MENU_BG,
    ...
    >
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 8 }}>
        {showCryptoIcon ? (
          ...
          <CivMarketIcon title={marketApi.question} category={marketApi.category} />
        ) : resolvedImageUrl ? (
          <div
            style={{
              width: 40,
...
const zionbetActionConfirmShellStyle: CSSProperties = {
  marginTop: "12px",
  width: "100%",
  boxSizing: "border-box",
...
function ZionBetActionConfirmShell({
  title,
  onClose,
  children,
}: {
  ...
  return (
    <motion.div
      ...
      style={zionbetActionConfirmShellStyle}
    >
      <button
        type="button"
        onClick={onClose}
      ...
      <div style={{ color: "white", fontSize: "0.9rem", lineHeight: 1.8 }}>{children}</div>
...
function ZionBetBuyConfirmCard({
  ...
  return (
    <ZionBetActionConfirmShell title="✅ Bet Placed!" onClose={onClose}>
      <ZionBetConfirmRow delay={0.05}>
        Side:{" "}
        <span style={{ color: bet.direction ? "#00ff88" : "#ff4444", fontWeight: "bold" }}>
          {bet.direction ? "YES" : "NO"}
        </span>
      </ZionBetConfirmRow>
      <ZionBetConfirmRow delay={0.1}>
        Amount:{" "}
        <strong>
          {bet.amount} {bet.currency}
        </strong>
      </ZionBetConfirmRow>
      <ZionBetConfirmRow delay={0.15}>
        Price: <strong>{bet.odds}¢</strong>
      </ZionBetConfirmRow>
      <ZionBetConfirmRow delay={0.2}>
        Shares: <strong>{shares.toFixed(2)}</strong>
      </ZionBetConfirmRow>
      <ZionBetConfirmRow delay={0.25}>
        Potential win:{" "}
        <strong style={{ color: "#00ff88" }}>
          {bet.payout.toFixed(2)} {bet.currency}
        </strong>
      </ZionBetConfirmRow>
    </ZionBetActionConfirmShell>
  );
}
...
function ZionBetSellConfirmCard({
  ...
  const pnl = bet.received - bet.stake;
  return (
    <ZionBetActionConfirmShell title="✅ Position Closed!" onClose={onClose}>
      <ZionBetConfirmRow delay={0.05}>
        Side:{" "}
        <span style={{ color: bet.direction ? "#00ff88" : "#ff4444", fontWeight: "bold" }}>
          {bet.direction ? "YES" : "NO"}
      ...
      <ZionBetConfirmRow delay={0.1}>
        Shares sold: <strong>{bet.sharesSold.toFixed(2)}</strong>
      </ZionBetConfirmRow>
      <ZionBetConfirmRow delay={0.15}>
        Received:{" "}
        <strong style={{ color: "#00ff88" }}>
          {bet.received.toFixed(2)} {bet.currency}
        </strong>
      </ZionBetConfirmRow>
      <ZionBetConfirmRow delay={0.2}>
        P&L:{" "}
        <strong style={{ color: pnl >= 0 ? "#00ff88" : "#ff4444" }}>
          {pnl >= 0 ? "+" : ""}
          {pnl.toFixed(2)} {bet.currency}
        </strong>
      </ZionBetConfirmRow>
    </ZionBetActionConfirmShell>
  );
    ...
    try {
      ...
      await zionbetExecuteClosePosition(positionForClose, walletAddress, signAndExecute, {
        onSuccess: (message, type, meta) => {
          ...
          if (type === "success") {
            const received =
              typeof meta?.payout === "number" && Number.isFinite(meta.payout)
                ? meta.payout
                : zionbetEarlyCloseReturnSui(userPosition.amount_sui);
            setSellConfirm({
              direction: userPosition.direction === "YES",
              currency: positionCurrency,
              sharesSold: posTotalShares,
              received,
              stake: userPosition.amount_sui,
            });
            if (onPositionClosed) {
              onPositionClosed({ message, type, silent: true });
            }
          } else if (onPositionClosed) {
          ...
          <aside className="zbMarketDetailSidebar">
            ...
            >
              ...
              {panelSide === "buy" ? (
                <>
                  ...
                  <div
                    ...
                    style={{
                      marginBottom: "12px",
                      position: "relative",
                      zIndex: currencyMenuOpen ? 30 : "auto",
                    }}
                  >
                <div style={{ ...zionbetDetailInnerPanel(), padding: 0, position: "relative", zIndex: 1 }}>
                <button
                  type="button"
                ...
                {currencyMenuOpen ? (
                  <div
                    style={zionbetCurrencyDropdownPanel({
                      position: "absolute",
                      top: "calc(100% + 6px)",
                      left: 0,
                      right: 0,
                    ...
                    {(["SUI", "USDC"] as const).map((c) => (
                      <button
                        ...
                        style={{
                          ...
                          border: "none",
                          borderBottom:
                            c === "SUI" ? "1px solid rgba(100, 180, 255, 0.18)" : "none",
                          background:
                            betCurrency === c ? "#123a5c" : ZIONBET_CURRENCY_MENU_BG,
                          color: "#ffffff",
                          fontSize: "13px",
                          fontWeight: 600,
                          cursor: "pointer",
...
/** Sidebar duration buckets — All through Long-term. */
const ZIONBET_CRYPTO_TIMEFRAME_SIDEBAR: { icon: string; label: string; tf: string }[] = [
  ...
  { icon: "📊", label: "Weekly", tf: "7d" },
  { icon: "📈", label: "Monthly", tf: "30d" },
  { icon: "🗓️", label: "Long-term", tf: "1y" },
];

const ZIONBET_DURATION_BUCKETS = ["15m", "1h", "24h", "7d", "30d", "1y"] as const;
type ZionbetDurationBucket = (typeof ZIONBET_DURATION_BUCKETS)[number];

function zionbetTimeframeFromMarketDates(
  endDate?: string | null,
  createdAt?: string | null
): ZionbetDurationBucket {
  if (endDate && createdAt) {
    const end = new Date(endDate).getTime();
    const start = new Date(createdAt).getTime();
    if (Number.isFinite(end) && Number.isFinite(start) && end > start) {
      return zionbetDurationBucketFromMs(end - start);
    }
  }
  if (endDate) {
    const end = new Date(endDate).getTime();
    if (Number.isFinite(end)) {
      return zionbetDurationBucketFromMs(Math.max(end - Date.now(), 0));
    }
  }
  return "24h";
}

function zionbetDurationBucketFromMs(ms: number): ZionbetDurationBucket {
  if (ms <= 20 * 60 * 1000) return "15m";
  if (ms <= 90 * 60 * 1000) return "1h";
  if (ms <= 36 * 60 * 60 * 1000) return "24h";
  if (ms <= 7 * 24 * 60 * 60 * 1000) return "7d";
  if (ms <= 30 * 24 * 60 * 60 * 1000) return "30d";
  return "1y";
}

function zionbetNormalizeDurationTimeframe(tf: string): ZionbetDurationBucket | null {
  const key = zionBetTfKeyFromZionMarket(tf);
  if (key === "15m" || key === "1h" || key === "24h" || key === "7d" || key === "30d" || key === "1y") {
    return key;
  }
  if (key === "4h") return "24h";
  return null;
}

function zionbetMarketDurationBucket(m: ZionbetApiMarket): ZionbetDurationBucket {
  if (m.timeframe) {
    const bucket = zionbetNormalizeDurationTimeframe(m.timeframe);
    if (bucket) return bucket;
  }
  if (m.end_date && m.created_at) {
...
function zionbetMarketMatchesBetTimeframe(m: ZionbetApiMarket, betTimeframe: string): boolean {
  if (betTimeframe === "all") return true;
  return zionbetMarketDurationBucket(m) === betTimeframe;
}
...
function executeDeepBookMintBinary(
  _signAndExecute: SignAndExecuteMutateFn,
  _params: {
    oracleId: string;
    strike: bigint;
    expiry: bigint;
  ...
  callbacks.onError(
    "DeepBook mint now uses the amount modal — click CALL/PUT and confirm in the dialog."
  );
}
...
export function ZionHome({
  ...
  const navigateBackToPeMarkets = useCallback(() => {
    let tab: ZionbetBetTab = betTab;
    if (typeof window !== "undefined") {
      tab = parseZionbetPeTabParam(new URLSearchParams(window.location.search).get("tab")) ?? betTab;
    }
    router.push(zionbetPeListHref(tab));
  }, [router, betTab]);

  useEffect(() => {
    if (!standalone || typeof window === "undefined") return;
    const fromUrl = parseZionbetPeTabParam(new URLSearchParams(window.location.search).get("tab"));
    if (fromUrl) setBetTab(fromUrl);
  }, [standalone]);
  ...
  const [deepbookPredictManagerId, setDeepbookPredictManagerId] = useState<string | null>(null);
  const [deepbookPredictManagerResolving, setDeepbookPredictManagerResolving] = useState(false);
  const [deepbookPredictManagerCreating, setDeepbookPredictManagerCreating] = useState(false);
  const [deepbookMintModal, setDeepbookMintModal] = useState<{
    oracleId: string;
    asset: string;
  ...
  const [deepbookMintAmount, setDeepbookMintAmount] = useState("10");
  const [deepbookMintPreview, setDeepbookMintPreview] = useState<{
    quantity: string;
    mintCostDusdc: string;
    redeemPayoutDusdc: string;
  } | null>(null);
  const [deepbookMintPreviewLoading, setDeepbookMintPreviewLoading] = useState(false);
  const [deepbookMintSubmitting, setDeepbookMintSubmitting] = useState(false);
  ...
  useEffect(() => {
    if (!account?.address) {
      setDeepbookPredictManagerId(null);
      return;
    }
    let cancelled = false;
    setDeepbookPredictManagerResolving(true);
    resolvePredictManagerId(suiClientHook, account.address)
      .then((id) => {
        if (!cancelled) setDeepbookPredictManagerId(id);
      })
      .catch(() => {
        if (!cancelled) setDeepbookPredictManagerId(null);
      })
      .finally(() => {
        if (!cancelled) setDeepbookPredictManagerResolving(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleCreateDeepBookPredictManager = useCallback(() => {
    if (!account?.address) {
      setZionBetNotify({ message: "Connect wallet first", type: "error" });
      return;
    }
    if (deepbookPredictManagerCreating) return;
    setDeepbookPredictManagerCreating(true);
      ...
      { transaction: tx, chain: "sui:testnet" },
      {
        onSuccess: async (result) => {
          try {
            ...
            const block = await waitForPredictManagerTx(suiClientHook, digest);
            const managerId = extractPredictManagerId(block);
            if (!managerId) {
              throw new Error("PredictManager ID not found in transaction effects");
            }
            setStoredPredictManagerId(account.address, managerId);
            setDeepbookPredictManagerId(managerId);
            setZionBetNotify({
              message: `PredictManager ready (${managerId.slice(0, 10)}…)${digest ? ` · TX ${digest.slice(0, 12)}…` : ""}`,
              type: "success",
            });
          } catch (err) {
            setZionBetNotify({
            ...
            setDeepbookPredictManagerCreating(false);
          }
        },
        onError: (err) => {
          setDeepbookPredictManagerCreating(false);
          setZionBetNotify({ message: err.message, type: "error" });
        },
      }
    );
  }, [
  ...
  const openDeepbookOracleMintModal = useCallback(
    (
      oracle: {
        oracle_id: string;
        underlying_asset: string;
      ...
      const spot = oracle.spot_price ?? 0;
      setDeepbookMintModal({
        oracleId: oracle.oracle_id,
        asset: oracle.underlying_asset,
        spotPrice: oracle.spot_price,
        expiryMs: oracle.expiry,
        strike: deepBookStrikeFromSpot(spot, isCall),
        isUp: isCall,
        sideLabel: isCall ? "CALL +5%" : "PUT -5%",
      });
      setDeepbookMintAmount("10");
      setDeepbookMintPreview(null);
    },
    [account?.address]
  );

  useEffect(() => {
    if (!deepbookMintModal || !account?.address) {
      setDeepbookMintPreview(null);
      return;
    }
    const amount = Number(deepbookMintAmount);
    if (!Number.isFinite(amount) || amount <= 0) {
      setDeepbookMintPreview(null);
      return;
    }
    let cancelled = false;
    const budget = dusdcToBaseUnits(amount);
    setDeepbookMintPreviewLoading(true);
    estimateQuantityForBudget(
      suiClientHook,
      account.address,
      {
        oracleId: deepbookMintModal.oracleId,
        expiryMs: BigInt(deepbookMintModal.expiryMs),
        strike: deepbookMintModal.strike,
        isUp: deepbookMintModal.isUp,
      },
      budget
    )
      .then((preview) => {
        if (cancelled) return;
        if (preview.quantity <= BigInt(0)) {
          setDeepbookMintPreview(null);
          return;
        }
        setDeepbookMintPreview({
          quantity: preview.quantity.toString(),
          mintCostDusdc: formatDusdcBaseUnits(preview.mintCost),
          redeemPayoutDusdc: formatDusdcBaseUnits(preview.redeemPayout),
        });
      })
      .catch(() => {
        if (!cancelled) setDeepbookMintPreview(null);
      })
      .finally(() => {
        if (!cancelled) setDeepbookMintPreviewLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [deepbookMintModal, deepbookMintAmount, account?.address, suiClientHook]);

  const handleConfirmDeepbookMint = useCallback(() => {
    if (!deepbookMintModal || !account?.address) return;
    const amount = Number(deepbookMintAmount);
    if (!Number.isFinite(amount) || amount <= 0) {
      setZionBetNotify({ message: "Enter a valid DUSDC amount", type: "error" });
      return;
    }
    if (!deepbookMintPreview || BigInt(deepbookMintPreview.quantity) <= BigInt(0)) {
      setZionBetNotify({
        message: "Amount too small for this strike — increase DUSDC or wait for preview",
        type: "error",
      });
      return;
    }
    if (deepbookMintSubmitting) return;

    setDeepbookMintSubmitting(true);
    void (async () => {
      try {
        const coins = await getDusdcCoins(suiClientHook, account.address);
        if (!coins.data.length) {
          throw new Error(
            "No testnet DUSDC in wallet. Request DUSDC from https://tally.so/r/Xx102L"
          );
        }

        const budget = dusdcToBaseUnits(amount);
        const quantity = BigInt(deepbookMintPreview.quantity);
        const managerId =
          deepbookPredictManagerId ??
          (await resolvePredictManagerId(suiClientHook, account.address));

        const { tx, createsManager } = buildDeepBookPredictMintTx({
          ...
          isUp: deepbookMintModal.isUp,
          quantity,
          depositAmountBase: budget,
          dusdcCoinIds: coins.data.map((c) => c.coinObjectId),
        });
          ...
          { transaction: tx, chain: "sui:testnet" },
          {
            onSuccess: async (result) => {
              try {
                ...
                if (createsManager || !managerId) {
                  const block = await waitForPredictManagerTx(suiClientHook, digest);
                  const newManagerId = extractPredictManagerId(block);
                  if (newManagerId) {
                    setStoredPredictManagerId(account.address, newManagerId);
                    setDeepbookPredictManagerId(newManagerId);
                  }
                }
                setDeepbookMintModal(null);
                setZionBetNotify({
                  message: `DeepBook ${deepbookMintModal.sideLabel} minted · qty ${quantity.toString()} · TX ${digest.slice(0, 14)}…`,
                  type: "success",
                });
              } catch (err) {
                setZionBetNotify({
                ...
                setDeepbookMintSubmitting(false);
              }
            },
            onError: (err) => {
              setDeepbookMintSubmitting(false);
  ...
  useEffect(() => {
    ...
    fetch(`/api/zionbet/market/${encodeURIComponent(standaloneMarketId)}`, { cache: "no-store" })
      ...
      .then((row) => {
        if (cancelled) return;
        const polyMarketId = row.market_id ?? row.id;
        if (row.error || !polyMarketId) {
          return fetch("/api/zion-markets", { cache: "no-store" })
            .then((r) => r.json())
            .then((data: { success?: boolean; markets?: ZionMarketRow[] }) => {
              if (cancelled) return;
              const markets = Array.isArray(data.markets) ? data.markets : [];
              const found = markets.find((m) => m.market_id === standaloneMarketId);
              if (found) {
                setStandaloneMarket(zionMarketRowToApiMarket(found));
                setStandaloneMarketError(null);
              } else if (!fromLists) {
  ...
  const zionbetTabMarketsForTimeframe = useMemo(() => {
    if (betTab === "civilization") {
      return [
        ...zionbetMarkets.civilization,
        ...zionMarkets.map(zionMarketRowToApiMarket),
      ];
    }
    if (betTab === "crypto") {
      return [...DEEPBOOK_BINARY_MARKETS, ...(polyByTab.crypto ?? [])];
    }
    if (POLY_TABS.includes(betTab)) {
      return [...(polyByTab[betTab] ?? [])];
    }
    return [];
  }, [betTab, zionbetMarkets.civilization, zionMarkets, polyByTab]);

  const zionbetFilteredDeepbookMarkets = useMemo(() => {
    if (betTab !== "crypto") return [];
    if (betTimeframe === "all") return [...DEEPBOOK_BINARY_MARKETS];
    return DEEPBOOK_BINARY_MARKETS.filter((m) => zionbetMarketMatchesBetTimeframe(m, betTimeframe));
  }, [betTab, betTimeframe]);

  const betTimeframeCounts = useMemo((): Record<string, number> => {
    const counts: Record<string, number> = {
      all: zionbetTabMarketsForTimeframe.length,
      "15m": 0,
      "1h": 0,
      "24h": 0,
      "7d": 0,
    ...
    for (const m of zionbetTabMarketsForTimeframe) {
      const bucket = zionbetMarketDurationBucket(m);
      counts[bucket] += 1;
    }
    return counts;
  }, [zionbetTabMarketsForTimeframe]);

  const zionbetFilteredZionMarkets = useMemo(() => {
    if (betTab !== "civilization") return [];
    if (betTimeframe === "all") return zionMarkets;
    return zionMarkets.filter((row) =>
      zionbetMarketMatchesBetTimeframe(zionMarketRowToApiMarket(row), betTimeframe)
    );
  }, [betTab, betTimeframe, zionMarkets]);
  ...
  const zionbetDisplayedMarkets = useMemo(() => {
    ...
    if (betTimeframe !== "all") {
      return sorted.filter((m) => zionbetMarketMatchesBetTimeframe(m, betTimeframe));
    }
    return sorted;
  ...
  const tabCtx: ZionTabContextValue = {
    ...
    deepbookMintModal,
    deepbookMintAmount,
    setDeepbookMintAmount,
    deepbookMintPreview,
    deepbookMintPreviewLoading,
    deepbookMintSubmitting,
    openDeepbookOracleMintModal,
    handleConfirmDeepbookMint,
    setDeepbookMintModal,
    ...
    <main className="page" style={standalone ? { background: "transparent", position: "relative", zIndex: 1 } : undefined}>
              ...
              <div className="dashboard show" style={isMobile ? { padding: "8px 16px" } : undefined}>
                ...
                {standaloneMarketLoading && !standaloneMarket ? (
                  ...
                  <GlassCard className={glassCardStyles.glassCardLab} style={{ padding: "24px", textAlign: "center" }}>
                    ...
                    <button
                      type="button"
                      onClick={navigateBackToPeMarkets}
                      style={{
                        cursor: "pointer",
                        color: "#ffffff",
                        fontSize: "14px",
                  ...
                  <ZionBetMarketDetailOverlay
                    ...
                    onClose={navigateBackToPeMarkets}
        ...
        ? createPortal(
            <ZionBetMyBetsOverlay
              ...
              onOpenMarket={(m) => {
                ...
                router.push(zionbetPeMarketHref(m.id, betTab));
              }}
              onOpenMarketId={(marketId) => {
                setShowMyBetsOverlay(false);
                router.push(zionbetPeMarketHref(marketId, betTab));
              }}
              onPositionClosed={handlePositionClosed}
            />,
            document.body
      ...
      {deepbookMintModal ? (
        <div
          ...
          onClick={() => !deepbookMintSubmitting && setDeepbookMintModal(null)}
        >
          <div onClick={(e) => e.stopPropagation()}>
            <div
              style={{
            ...
            >
              <div style={{ marginBottom: "16px" }}>
                ...
                >
                  DEEPBOOK PREDICT · {deepbookMintModal.asset}/USD
                </div>
                <div
                  style={{
                    color: "#f1f5f9",
              ...
              <input
                ...
                min="0"
                step="0.01"
                value={deepbookMintAmount}
                onChange={(e) => setDeepbookMintAmount(e.target.value)}
                disabled={deepbookMintSubmitting}
                style={{
                  width: "100%",
                  boxSizing: "border-box",
                  padding: "12px",
              ...
              >
                {deepbookMintPreviewLoading ? (
                  "Calculating preview via get_trade_amounts…"
                ) : deepbookMintPreview ? (
                  <>
                    <div style={{ color: "#4DA2FF", marginBottom: "6px" }}>Preview</div>
                    <div>
                      For <strong style={{ color: "#f1f5f9" }}>{deepbookMintAmount} DUSDC</strong>{" "}
                      → mint <strong style={{ color: "#00ff41" }}>{deepbookMintPreview.quantity}</strong>{" "}
                      contract(s)
                    </div>
                    <div>Mint cost ≈ {deepbookMintPreview.mintCostDusdc} DUSDC</div>
                    <div>Redeem payout (now) ≈ {deepbookMintPreview.redeemPayoutDusdc} DUSDC</div>
                    {!deepbookPredictManagerId ? (
                      <div style={{ marginTop: "8px", color: "#ff9f43" }}>
                        PredictManager will be created in the same transaction.
                      </div>
                    ) : null}
                  </>
                ) : (
                  "Enter a DUSDC amount to preview quantity and cost."
                )}
              </div>
              ...
              <button
                ...
                disabled={
                  ...
                  deepbookMintPreviewLoading ||
                  !deepbookMintPreview ||
                  BigInt(deepbookMintPreview?.quantity ?? "0") <= BigInt(0)
                }
                onClick={() => void handleConfirmDeepbookMint()}
                style={{
                  width: "100%",
                  padding: "14px",
                  borderRadius: "8px",
                  cursor:
                    deepbookMintSubmitting || deepbookMintPreviewLoading
                      ? "wait"
                      : "pointer",
                  fontFamily: "monospace",
                  fontSize: "0.9rem",
                  fontWeight: "bold",
                  opacity:
                    deepbookMintSubmitting || deepbookMintPreviewLoading ? 0.6 : 1,
                  background: deepbookMintModal.isUp
                    ? "rgba(0,255,65,0.2)"
                    : "rgba(255,65,65,0.2)",
                  border: "1px solid rgba(255, 255, 255, 0.15)",
                  color: deepbookMintModal.isUp ? "#00ff41" : "#ff4141",
                }}
              >
                {deepbookMintSubmitting
                  ? "Signing…"
                  : `Confirm ${deepbookMintModal.sideLabel}`}
              </button>
            </div>
          </div>
        </div>
