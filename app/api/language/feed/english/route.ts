...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${API_BASE}/language/feed/english?limit=${encodeURIComponent(limit)}`, {
    ...
    return NextResponse.json({ error: "Failed to fetch English feed" }, { status: 502 });
  }
}
