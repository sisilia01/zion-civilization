...
PRESIDENT_SALARY = 200.0
SHERIFF_SALARY = 150.0
SALARY_COOLDOWN_HOURS = 20
...
def emergency_corp_lending(cur) -> float:
    """During unemployment crisis, issue relaxed emergency loans to active corporations."""
    cur.execute(
        """
    ...
    reserve = zrs_reserve(cur)
    emergency_loan = 800.0
    max_outstanding = reserve * MAX_LOANS_RESERVE_PCT
    cur.execute("SELECT COALESCE(SUM(principal), 0) AS s FROM zrs_loans WHERE is_active = true")
    outstanding = float((cur.fetchone() or {}).get("s") or 0)
    if outstanding >= max_outstanding:
        return 0.0
    ...
    issued = 0
    for corp in cur.fetchall():
        if zrs_reserve(cur) < emergency_loan * 2:
            break
        if outstanding + emergency_loan > max_outstanding:
            break
        if not zrs_deduct_reserve(cur, emergency_loan):
            break
        cur.execute(
            "UPDATE corporations SET treasury = treasury + %s WHERE id = %s",
            (emergency_loan, corp["id"]),
        )
        cur.execute(
            """
            INSERT INTO zrs_loans (
        ...
        log_event(
            ...
            None,
            ZRS_EVENT_TYPE,
            f"ZRS Emergency Lending: {corp['name']} +{emergency_loan:.0f} ZION "
            f"(unemployment crisis {unemployment:.0f}%)",
            emergency_loan,
            priority="urgent",
        )
        outstanding += emergency_loan
        lent += emergency_loan
        issued += 1

    if issued > 0:
        print(f"  🚨 Emergency corp lending: {issued} loans, {lent:.0f} ZION")
    return lent
...
def execute_frs_directive(cur, ctx: dict | None = None) -> dict:
    ...
    cur.execute(
        """
        SELECT pending_directive, chief_name, is_active, confirmation_status
        FROM frs_chief_state WHERE id = 1
        """
    )
    row = cur.fetchone() or {}
    if not row.get("is_active"):
        if (row.get("confirmation_status") or "") == "vacant":
            log_event(
                cur,
                None,
                ZRS_EVENT_TYPE,
                "ZRS: no_active_frs_chief — FRS Chief office vacant, awaiting nomination",
                0,
                priority="normal",
            )
    ...
    if action == "tax_change":
        ...
        if moved == 0:
            headline = (
                f"FRS QE SKIPPED: no corporations eligible "
                f"(all treasury >= 5000 threshold)"
            )
        else:
...
def run_zrs_maintenance(cur):
    ...
    emergency_flow = emergency_corp_lending(cur)
    ...
    log_event(
        ...
        f"loans {loan_flow:,.0f} + emergency {emergency_flow:,.0f} | {subsidized} trading subsidies",
        loan_flow + emergency_flow,
        priority="normal",
    )


def pay_regular_salaries(cur):
    """Daily appropriation to president/sheriff from ZRS reserve with cooldown."""
    cur.execute(
        "ALTER TABLE zrs_state ADD COLUMN IF NOT EXISTS last_salary_paid_at TIMESTAMP"
    )
    cur.execute(
        """
        SELECT reserve, last_salary_paid_at
        FROM zrs_state
        WHERE id = 1
        """
    )
    ...
    last_paid = row.get("last_salary_paid_at")
    total = PRESIDENT_SALARY + SHERIFF_SALARY

    if last_paid is not None:
        cur.execute(
            "SELECT (NOW() - %s) >= INTERVAL '%s hours' AS ready",
            (last_paid, SALARY_COOLDOWN_HOURS),
        )
        ready = bool((cur.fetchone() or {}).get("ready"))
        if not ready:
            return False
    ...
    cur.execute(
        "UPDATE zrs_state SET last_salary_paid_at = NOW(), updated_at = NOW() WHERE id = 1"
    )
    log_event(
        cur,
        None,
        "zrs",
        f"💰 ZRS daily appropriation: President +{PRESIDENT_SALARY:.0f}, "
        f"Sheriff +{SHERIFF_SALARY:.0f} ZION",
        total,
        priority="normal",
    )
    return True
...
def main():
    ...
    from frs_chief import auto_nominate_if_long_vacant

    if auto_nominate_if_long_vacant(cur):
        print("  FRS auto-nomination triggered (vacant 3+ ticks)", flush=True)

    result = execute_frs_directive(cur)
    print(f"  FRS directive: {result.get('headline') or result.get('reason', '—')}")
    pay_regular_salaries(cur)
