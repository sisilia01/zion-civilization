...
def assign_unread_chunks_batch(
    ...
    """
    Assign one per-agent unread chunk per agent, filtered by specialization.
    30% core curriculum track, 70% agent specialty track.
    """
    if not agent_ids:
        return {}
    ...
    for agent_id in agent_ids:
        cur.execute(
            "SELECT specialization_track FROM agents WHERE id = %s",
            (agent_id,),
        )
        row = cur.fetchone()
        specialty = (row.get("specialization_track") if row else None) or "PHILOSOPHY"

        if random.random() < 0.3:
            track_filter = random.choice(CORE_TRACKS)
        else:
            track_filter = specialty
        ...
        cur.execute(
            ...
            LIMIT 1
            """,
            (track_filter, agent_id),
        )
        chunk = cur.fetchone()
        if chunk:
            assignments[agent_id] = dict(chunk)
