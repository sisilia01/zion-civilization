...
SHERIFF_IMPEACH_FAILURE_MIN = 70
...
def president_prior_terms(cur, agent_id: int) -> int:
    """How many times this agent has held the presidency (any phase)."""
    cur.execute(
        "SELECT COUNT(*) AS c FROM president_state WHERE agent_id = %s",
        (agent_id,),
    )
    return int((cur.fetchone() or {}).get("c") or 0)


def is_president_reelection_blocked(cur, agent_id: int) -> bool:
    """True when constitutional max_consecutive_terms bars another presidency."""
    max_terms = float(get_param("max_consecutive_terms", 999))
    if max_terms >= 999:
        return False
    return president_prior_terms(cur, agent_id) >= int(max_terms)
...
def pick_senate_president_nominee(cur, party_id: str) -> dict | None:
    ...
    cur.execute(
        ...
        SELECT s.id AS senate_id, s.agent_id AS id, s.agent_name AS name,
               s.approval_rating, a.class, a.charisma, a.balance, a.party,
    ...
    for row in cur.fetchall():
        if not check_cooling_off(cur, row["id"], "president"):
            log_cooling_off_block(
                cur,
                row["id"],
                row["name"],
            ...
            continue
        if is_president_reelection_blocked(cur, row["id"]):
            log_cooling_off_block(
                cur,
                row["id"],
                row["name"],
                "president",
                "Term Limits Act — single term, no re-election",
            )
            continue
...
def pick_sheriff_party_nominee(cur, party_id: str, exclude_ids: set | None = None) -> dict | None:
    ...
    cur.execute(
        f"""
        SELECT a.id, a.name, a.class, a.charisma, a.balance, a.party,
               s.party_id AS senate_party,
    ...
    for row in rows:
        if _agent_party_for_election(row) != party_id:
            continue
        if not check_cooling_off(cur, row["id"], "sheriff"):
            log_cooling_off_block(
                cur,
                row["id"],
                row["name"],
...
def _fetch_senator_candidates(
    ...
    *,
    ...
    """Living party members eligible for senate election (agents.party is source of truth)."""
    exclude_ids = exclude_ids or set()
    ex_clause = ""
    ...
    cur.execute(
        f"""
        SELECT id, name, charisma, class, balance, party
        FROM agents
        WHERE is_alive = true
          AND party = %s
...
def _log_no_senator_candidates(cur, party_id: str) -> None:
    log_event(
        cur,
        None,
        "senate",
        f"no senator candidates for party={party_id}",
        0,
        priority="urgent",
    )
...
def pick_senator_candidate(cur, party_id: str, exclude_ids: set | None = None) -> dict | None:
    rows = _fetch_senator_candidates(cur, party_id, exclude_ids, limit=1)
    if not rows:
        _log_no_senator_candidates(cur, party_id)
        return None
    return rows[0]
...
def _score_senate_candidate(agent: dict, party_id: str, class_weights: dict[str, float]) -> float:
    ...
    agent_party = _agent_party_for_election(agent)
    party_bonus = 1.25 if agent_party == party_id else 0.85
...
def elect_senator_by_popular_vote(
    ...
    candidates = _fetch_senator_candidates(cur, party_id, exclude_ids, limit=8)
    if not candidates:
        _log_no_senator_candidates(cur, party_id)
        return None
...
def _assign_senate_speaker(cur) -> None:
    ...
    speaker = None
    for row in cur.fetchall():
        if check_cooling_off(cur, row["agent_id"], "speaker"):
            speaker = row
            break
        log_cooling_off_block(
            cur,
...
def _seat_senator(
    ...
    cur.execute(
        """
        INSERT INTO senate (
            agent_id, agent_name, party_id, role, approval_rating, is_active, term_start
...
def get_sheriff_failure_score(cur) -> float:
    """Dereliction-of-duty score for Senate sheriff impeachment."""
    cur.execute(
        """
        SELECT police_count, police_budget, sheriff_type, started_at
    ...
    police_count = int(row.get("police_count") or 0)
    police_budget = float(row.get("police_budget") or 0)
    sheriff_type = (row.get("sheriff_type") or "honest").lower()
    sheriff_started = row.get("started_at")

    score = 0.0
    if police_count < 10:
        score += 30
    elif police_count < 20:
        score += 15
    ...
    if sheriff_started:
        cur.execute(
            ...
            WHERE description ILIKE '%%tipped off gangs%%'
              AND created_at > %s
            """,
            (sheriff_started,),
        )
    ...
    corrupt_incidents = int((cur.fetchone() or {}).get("c") or 0)
    if corrupt_incidents > 0:
        score += corrupt_incidents * 20
    if sheriff_type == "corrupt" and corrupt_incidents >= 2:
        score += 30

    cur.execute(
        """
        SELECT COALESCE(SUM(treasury), 0) AS gang_treasury
        FROM clans WHERE members_count > 0
        """
    )
    gang_treasury = float((cur.fetchone() or {}).get("gang_treasury") or 0)
    if police_budget > 0 and gang_treasury > police_budget:
        score += 40
    elif police_budget <= 0 and gang_treasury > 1000:
        score += 40

    return score


def check_sheriff_impeachment(cur) -> bool:
    """Senate impeachment of Sheriff for dereliction of duty (not revolution-based recall)."""
    from civ_governance import deactivate_sheriff

    failure_score = get_sheriff_failure_score(cur)
    if failure_score < SHERIFF_IMPEACH_FAILURE_MIN:
        return False
    ...
    votes_for = 0
    sheriff_corrupt = (sheriff.get("sheriff_type") or "").lower() == "corrupt"
    for s in senators:
        vote_chance = 0.55 + min(0.35, failure_score / 200.0)
        if sheriff_corrupt:
            vote_chance += 0.15
        if int(s.get("approval_rating") or 50) < 40:
            vote_chance += 0.10
        if random.random() < min(0.95, vote_chance):
            votes_for += 1

    threshold = max(1, senate_size // 2 + 1)
    if votes_for < threshold:
        log_event(
            cur,
            sheriff.get("agent_id"),
            "senate",
            f"Sheriff impeachment motion FAILED {votes_for}-{senate_size - votes_for} "
            f"(needs {threshold}; dereliction score {failure_score:.0f})",
            0,
            priority="urgent",
        )
        return False
    ...
    sname = sheriff.get("agent_name") or "Sheriff"
    sid = sheriff.get("agent_id")
    deactivate_sheriff(cur)
    log_event(
        cur,
        sid,
        "senate",
        f"⚖️ SHERIFF IMPEACHED: {sname} removed by Senate vote "
        f"({votes_for}-{senate_size - votes_for}) for dereliction of duty "
        f"(failure score {failure_score:.0f}) — new election called",
        0,
        priority="breaking",
    )
    return True
...
def check_expired_senator_terms(cur) -> int:
    """Re-elect individual senators when real-time term expires (staggered)."""
    senate_term_days = float(get_param("senate_term_days", 6))
    cur.execute(
        """
        SELECT s.id, s.agent_id, s.agent_name, s.party_id, s.role, s.term_start
        FROM senate s
        INNER JOIN agents a ON a.id = s.agent_id AND a.is_alive = true
        WHERE s.is_active = true
        """
    ...
    renewed = 0
    for senator in cur.fetchall():
        days_in_office = get_real_days_elapsed(senator.get("term_start"))
        if days_in_office < senate_term_days:
            continue

        party_id = senator["party_id"]
        role = senator.get("role") or "senator"
        old_name = senator.get("agent_name") or "Senator"
        cur.execute("DELETE FROM senate WHERE id = %s", (senator["id"],))
        ...
        seated_ids = {int(r["agent_id"]) for r in cur.fetchall()}
        seated_ids |= active_president_agent_ids(cur)

        agent = elect_senator_by_popular_vote(cur, party_id, seated_ids)
        if agent and agent["id"] not in seated_ids:
            _seat_senator(
                cur,
                agent,
                party_id,
                seated_ids,
                reason=f"term renewal after {days_in_office:.1f} days",
                role=role,
            )
            log_event(
                cur,
                agent["id"],
                "senate",
                f"Senator {old_name} ({party_id}) term ended — "
                f"{agent['name']} elected by popular vote (staggered {senate_term_days:.0f}d term)",
                0,
                priority="normal",
            )
            renewed += 1
            ...
            log_event(
                cur,
                senator.get("agent_id"),
                "senate",
                f"Senator {old_name} ({party_id}) term ended — no replacement candidate found",
                0,
                priority="urgent",
            )
    return renewed
...
def should_run_scheduled_election(cur, president: dict) -> bool:
    ...
    if president.get("election_delayed"):
        ...
        return False
    term_limit_days = float(get_param("term_limit_days", 3))
    days_in_power = get_real_days_elapsed(president.get("started_at"))
    approval = int(president.get("approval_rating") or 50)
    return days_in_power >= term_limit_days or approval < 10
...
def run_governance_tick(cur, ctx: dict) -> dict:
    ...
    summary_parts = []
    renewed = check_expired_senator_terms(cur)
    if renewed:
        summary_parts.append(f"renewed {renewed} senator seat(s)")
    president = get_president(cur)
    ...
    if check_sheriff_impeachment(cur):
        summary_parts.append("sheriff impeached — election pending")
        ctx["sheriff_recall"] = True
    elif check_sheriff_recall(cur):
