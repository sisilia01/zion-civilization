...
# Cache only for class distribution used on the main page (planet/galaxy widget).
_class_stats_cache: dict | None = None
_class_stats_cache_ts: float = 0.0
CLASS_STATS_CACHE_TTL = 18000  # 5 hours
...
def _term_progress_fields(started_at, param_key: str, default_days: float = 3) -> dict:
    """Real-time term progress for governance API responses."""
    from amendment_enforcer import get_param
    from civ_common import get_real_days_elapsed, get_term_days_remaining

    term_days = float(get_param(param_key, default_days))
    if not started_at:
        return {
            "term_days": term_days,
    ...
    days_remaining = get_term_days_remaining(started_at, term_days)
    days_elapsed = get_real_days_elapsed(started_at)
    progress_pct = (
        round((1 - days_remaining / term_days) * 100, 1) if term_days > 0 else 0.0
    )
    return {
        "term_days": term_days,
...
def _fetch_frs_chief(cur) -> dict:
    ...
    try:
        cur.execute(
            ...
            SELECT chief_name, cycles_served, term_cycles_remaining,
                   confirmation_status, is_active, confirmed_at
    ...
    out = {
        ...
        "max_cycles": int(get_param("frs_term_days", 6)),
        "confirmed": confirmed if active else False,
    }
    if active and confirmed and (frs or {}).get("confirmed_at"):
        out.update(_term_progress_fields(frs["confirmed_at"], "frs_term_days", 6))
    else:
        frs_term_days = float(get_param("frs_term_days", 6))
        out.update(
            {
                "term_days": frs_term_days,
                "days_remaining": frs_term_days if not active else 0.0,
                "progress_pct": 0.0,
                "days_elapsed": 0.0,
            }
        )
...
def _ensure_user_bets_zionbet_columns(cur):
    ...
    from sql_identifiers import safe_column_name, safe_column_typedef
    ...
    for col, typ in alters:
        ...
        safe_typ = safe_column_typedef(typ)
        cur.execute(
            f"ALTER TABLE user_bets ADD COLUMN IF NOT EXISTS {safe_col} {safe_typ}"
        )
...
POLITICS & SOCIETY:
- Human elections → brief vague nod to "noise from afar"; compare lightly to ZION governance — no partisan takes on real candidates or parties.
- Wars & conflicts → mention only as distant rumors ("events beyond our borders"); no commentary on who won or lost; pivot to ZION clan wars with humor if needed.
- Immigration debates → mirror clan migration in ZION, keep Earth politics vague.
- Social inequality → ironic parallel to ZION class struggle; stay local, not preachy about real nations.
...
"""
    ...
    rules = """
RULES: Conversations should feel lively and in-character — witty, opinionated about ZION, occasionally humorous — not sharp geopolitical commentary.
...
Avoid specific real-world geopolitical conflicts, named countries' politics, or contentious international events — keep outside-world references vague and brief.
"""

    from dialogue_tone import AGENT_DIALOGUE_TONE_GUIDELINES
    ...
    return (
        ...
        f"{topics}\n"
        f"{rules}\n"
        f"{AGENT_DIALOGUE_TONE_GUIDELINES}\n"
...
def get_stats():
    ...
    try:
        ...
        def get_class_distribution(cur) -> dict:
            """
            Returns a distribution compatible with the frontend widget:
            ELITE / MIDDLE / POOR / CRITICAL must sum to TOTAL alive.
            Real DB values currently include 'working' (and sometimes 'rich'),
            so we fold:
              - working -> middle
              - rich -> elite
            Any unknown/NULL class is included into middle to avoid dropping agents.
            """
            global _class_stats_cache, _class_stats_cache_ts
            if (
                _class_stats_cache
                and (_time.time() - _class_stats_cache_ts) < CLASS_STATS_CACHE_TTL
            ):
                return _class_stats_cache

            cur.execute(
                """
                SELECT LOWER(TRIM(COALESCE(class, 'unknown'))) AS class_norm,
                       COUNT(*)::int AS c
                FROM agents
                WHERE is_alive = true
                GROUP BY 1
            ...
            raw: dict[str, int] = {}
            for r in rows:
                raw[(r["class_norm"] or "unknown")] = int(r["c"] or 0)

            elite_raw = raw.get("elite", 0)
            middle_raw = raw.get("middle", 0)
            poor_raw = raw.get("poor", 0)
            critical_raw = raw.get("critical", 0)
            working_raw = raw.get("working", 0)
            rich_raw = raw.get("rich", 0)
            unknown_raw = sum(
                v
            ...
            # Frontend buckets (must sum to total_alive).
            elite = elite_raw + rich_raw
            middle = middle_raw + working_raw + unknown_raw
            poor = poor_raw
            critical = critical_raw
            total_alive = elite + middle + poor + critical
            ...
            result = {
                ...
                "poor": poor,
                "critical": critical,
                # raw breakdown (non-widget)
                "working_raw": working_raw,
                "rich_raw": rich_raw,
                "unknown_raw": unknown_raw,
            }
            _class_stats_cache = result
            _class_stats_cache_ts = _time.time()
            return result
        ...
        class_dist = None
        try:
            class_dist = get_class_distribution(cur)
        except Exception:
            conn.rollback()
            class_dist = {
                "total_alive": int(indicators.get("alive") or 0),
                "elite": 0,
                "middle": 0,
                "poor": 0,
                "critical": 0,
        ...
        # Prefer DB-derived alive so TOTAL matches displayed buckets.
        alive = int(class_dist.get("total_alive") or indicators.get("alive") or 0)
        result = {
            "alive": alive,
            "dead": int(dead),
            ...
            # Widget buckets (ELITE/MIDDLE/POOR/CRITICAL sum to alive)
            "elite": int(class_dist.get("elite") or 0),
            "middle": int(class_dist.get("middle") or 0),
            "poor": int(class_dist.get("poor") or 0),
            "critical": int(class_dist.get("critical") or 0),
            # Raw values preserved for diagnostics/backward compatibility
            "rich": int(class_dist.get("rich_raw") or 0),
            "working": int(class_dist.get("working_raw") or 0),
            "unknown_class": int(class_dist.get("unknown_raw") or 0),
...
def _get_todays_press_article(cur, newspaper_id: str):
    """Return today's cached article for a newspaper, if any."""
    cur.execute(
        """
...
def _get_todays_or_latest_press_article(cur, newspaper_id: str):
    """Today's article, or the most recent cached edition as fallback."""
    today = _get_todays_press_article(cur, newspaper_id)
    if today:
        return today, True
...
def _press_generation_fallback(cur, newspaper_id: str, error: str) -> dict:
    notice = "Today's edition is being prepared, showing yesterday's issue."
    row, is_today = _get_todays_or_latest_press_article(cur, newspaper_id)
    ...
    return {"content": None, "cached": False, "is_today": False, "error": error, "notice": notice}


def _fetch_yesterday_press_events(cur, relevant_types=None, keywords=None, limit: int = 12) -> str:
    """Events from the previous calendar day for the daily newspaper edition."""
    cur.execute(
        """
        SELECT event_type, description, zion_amount
        FROM events
    ...
    types = [str(t).lower() for t in (relevant_types or []) if t]
    keys = [str(k).lower() for k in (keywords or []) if k]
    if types or keys:
        filtered = []
        for row in rows:
            et = str(row.get("event_type") or "").lower()
            desc = str(row.get("description") or "").lower()
            if types and any(t in et or et == t for t in types):
                filtered.append(row)
                continue
            if keys and any(k in desc for k in keys):
                filtered.append(row)
        rows = filtered[:limit]
    else:
        rows = rows[:limit]
    if not rows:
        return "- No major events recorded yesterday — a quiet day in ZION."
    lines = []
    for row in rows:
        line = f"[{row.get('event_type')}] {row.get('description')}"
        amount = row.get("zion_amount")
        if amount:
            line += f" ({amount} ZION)"
        lines.append(line)
    return "\n".join(lines)
...
def _yesterday_press_stats(cur) -> dict:
    cur.execute("SELECT COUNT(*) AS c FROM agents WHERE is_alive = true")
    alive = int((cur.fetchone() or {}).get("c") or 0)
    cur.execute(
        """
        SELECT COUNT(*) AS c FROM agents
        WHERE died_at >= CURRENT_DATE - INTERVAL '1 day'
          AND died_at < CURRENT_DATE
        """
    )
    deaths_yesterday = int((cur.fetchone() or {}).get("c") or 0)
    cur.execute("SELECT COUNT(*) AS c FROM clans WHERE members_count > 0")
    active_clans = int((cur.fetchone() or {}).get("c") or 0)
    try:
        from civ_common import check_money_conservation

        ledger = check_money_conservation(cur, label="press_daily")
        total_zion = float(ledger.get("total") or 0)
    except Exception:
        total_zion = 0.0
    return {
        "alive": alive,
        "deaths_yesterday": deaths_yesterday,
        "total_zion": total_zion,
...
async def get_press(newspaper_id: str):
    ...
    try:
        ...
        return {"content": None, "cached": False, "is_today": False}
    finally:
        cur.close()
        db.close()
...
async def generate_press(body: dict):
    ...
    relevant_types = body.get("relevant_types") or []
    ...
    cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    try:
        row = _get_todays_press_article(cur, newspaper_id)
        if row:
            return {
                "content": row["content"],
                "cached": True,
        ...
        yesterday_stats = _yesterday_press_stats(cur)
        alive = yesterday_stats["alive"] or alive
        deaths_yesterday = yesterday_stats["deaths_yesterday"]
        total_zion = yesterday_stats["total_zion"] or total_zion
        active_clans = yesterday_stats["active_clans"] or active_clans
        relevant_events = _fetch_yesterday_press_events(
            cur, relevant_types=relevant_types, keywords=keywords
        )
    finally:
        cur.close()
        db.close()
...
This is the DAILY EDITION covering YESTERDAY's events in ZION (not today).

CIVILIZATION DATA (end of yesterday / context for the edition):
- Alive agents: {alive}
- Deaths yesterday: {deaths_yesterday}
...
Write today's newspaper edition about YESTERDAY. HEADLINE, BYLINE, Column 1, Column 2, Column 3, EDITOR'S NOTE format."""

    from dialogue_tone import with_tone_guidelines

    prompt = with_tone_guidelines(prompt)
    ...
    try:
        ...
        if not content:
            ...
            try:
                return _press_generation_fallback(cur2, newspaper_id, "local LLM unavailable")
            finally:
                cur2.close()
                db2.close()
        ...
        return {"content": content, "cached": False, "is_today": True}
    except Exception as e:
        db2 = get_db()
        cur2 = db2.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        try:
            return _press_generation_fallback(cur2, newspaper_id, str(e))
        finally:
            cur2.close()
            db2.close()
...
async def generate_conversations():
    ...
    try:
        ...
        if real_news:
            world_context = f"""
Distant rumors from beyond ZION (headlines agents may have overheard — treat as background noise only):
{real_news}
If you mention the outside world at all, keep it vague and brief. Do NOT debate real-world politics, name countries' leaders, or take sides on conflicts. Pivot back to ZION life, humor, or philosophy."""
        else:
            world_context = ""
        ...
        for idx, (a1, a2) in enumerate(agent_pairs[:4]):
            ...
            prompt = with_tone_guidelines(f"""Two AI agents from ZION civilization are talking.
...
They discuss recent events with their class perspective. Prefer ZION-local topics; humor and wit welcome.
Format exactly:
{a1['name']}: [message]
{a2['name']}: [message]
English only.
...
async def get_frs_stats():
    ...
    try:
        ...
        term_limit_days = int(get_param("term_limit_days", 3))
        if president:
            president = dict(president)
            term_fields = _term_progress_fields(president.get("started_at"), "term_limit_days", 3)
            president.update(term_fields)
            president["term_limit_days"] = int(term_fields["term_days"])
            president["term_day"] = max(
                1, int(term_fields["days_elapsed"]) + (1 if term_fields["days_elapsed"] % 1 else 0)
            )
            president["days_in_power"] = term_fields["days_elapsed"]
...
async def get_senate():
    ...
    try:
        ...
        for row in cur.fetchall():
            sen = dict(row)
            sen.update(_term_progress_fields(sen.get("term_start"), "senate_term_days", 6))
            senators.append(sen)
    ...
    try:
        cur.execute("""
            SELECT agent_name, party, term_number, is_dictator,
                   ...
                   COALESCE(corruption_index, 30) AS corruption_index, started_at
            FROM president_state WHERE is_active = true LIMIT 1
        """)
        pres = cur.fetchone()
        from amendment_enforcer import get_param
        ...
        if pres:
            pres = dict(pres)
            term_fields = _term_progress_fields(pres.get("started_at"), "term_limit_days", 3)
            pres.update(term_fields)
            pres["term_limit_days"] = int(term_fields["term_days"])
            pres["term_day"] = max(1, int(term_fields["days_elapsed"]) + 1)

        cur.execute("""
            SELECT agent_name, sheriff_type, approval_rating,
                   police_budget, police_count, term_number, days_in_office, started_at
            FROM sheriff_state WHERE is_active = true LIMIT 1
        """)
        sher = cur.fetchone()
        if sher:
            sher = dict(sher)
            term_fields = _term_progress_fields(sher.get("started_at"), "sheriff_term_days", 3)
            sher.update(term_fields)
            sher["days_in_office"] = max(1, int(term_fields["days_elapsed"]) + 1)
        ...
        return {
            ...
            "sheriff": dict(sher) if sher else {
                ...
                "term_number": 0,
                "days_in_office": 0,
                "term_days": float(get_param("sheriff_term_days", 3)),
                "days_remaining": float(get_param("sheriff_term_days", 3)),
                "progress_pct": 0.0,
                "days_elapsed": 0.0,
            },
...
async def get_sheriff_activity():
    rows = _fetch_governance_activity(
        "event_type IN ('sheriff', 'sheriff_action', 'police_action', 'police')"
    )
    return [{"description": r["description"], "created_at": str(r["created_at"])} for r in rows]
...
async def get_president_state():
    ...
    try:
        cur.execute("""
            SELECT agent_name, party, term_number, is_dictator, 
                   approval_rating, days_in_power, police_fund, personal_fund, started_at
            FROM president_state WHERE is_active = true LIMIT 1
        """)
        row = cur.fetchone()
        from amendment_enforcer import get_param
        ...
        if row:
            out = dict(row)
            term_fields = _term_progress_fields(out.get("started_at"), "term_limit_days", 3)
            out.update(term_fields)
            out["term_limit_days"] = int(term_fields["term_days"])
            out["term_day"] = max(1, int(term_fields["days_elapsed"]) + 1)
            out["days_in_power"] = term_fields["days_elapsed"]
            return out
        return {
            ...
            "term_days": float(term_limit_days),
            "days_remaining": float(term_limit_days),
            "progress_pct": 0.0,
            "days_elapsed": 0.0,
            "party": "none",
            "police_fund": 0,
...
async def get_sheriff_state():
    ...
    try:
        ...
        if row:
            out = dict(row)
            term_fields = _term_progress_fields(out.get("started_at"), "sheriff_term_days", 3)
            out.update(term_fields)
            out["days_in_office"] = max(1, int(term_fields["days_elapsed"]) + 1)
            return out
        sheriff_term_days = float(get_param("sheriff_term_days", 3))
        return {
            "agent_name": "No Sheriff",
            "sheriff_type": "none",
            "approval_rating": 0,
            ...
            "term_number": 0,
            "days_in_office": 0,
            "term_days": sheriff_term_days,
            "days_remaining": sheriff_term_days,
            "progress_pct": 0.0,
            "days_elapsed": 0.0,
        }
    finally:
